...
private[ml] trait CusOneHotEncoderBase extends Params with HasHandleInvalid
  ...
  override val handleInvalid: Param[String] = new Param[String](this, "handleInvalid",
    ...
    ParamValidators.inArray(CusOneHotEncoderEstimator.supportedHandleInvalids))

  setDefault(handleInvalid, CusOneHotEncoderEstimator.ERROR_INVALID)
...
class CusOneHotEncoderEstimator @Since("2.3.0")(@Since("2.3.0") override val uid: String)
  ...
  override def transformSchema(schema: StructType): StructType = {
    val keepInvalid = $(handleInvalid) == CusOneHotEncoderEstimator.KEEP_INVALID
    validateAndTransformSchema(schema, dropLast = $(dropLast),
      keepInvalid = keepInvalid)
  }

  @Since("2.3.0")
  override def fit(dataset: Dataset[_]): CusOneHotEncoderModel = {
    ...
    val model = new CusOneHotEncoderModel(uid, categorySizes).setParent(this)
    copyValues(model)
  }

  @Since("2.3.0")
  override def copy(extra: ParamMap): CusOneHotEncoderEstimator = defaultCopy(extra)
}

@Since("2.3.0")
object CusOneHotEncoderEstimator extends DefaultParamsReadable[CusOneHotEncoderEstimator] {
  ...
  @Since("2.3.0")
  override def load(path: String): CusOneHotEncoderEstimator = super.load(path)
}
...
class CusOneHotEncoderModel(
  ...
  def setCategorySizes(value: Array[Int]): this.type = {
    categorySizes = value
    this
  }
        ...
        if (keepInvalid) {
          ...
          if (label < 0) {
            throw new SparkException(s"Negative value: $label. Input can't be negative. " +
              ...
              s"${CusOneHotEncoderEstimator.KEEP_INVALID}")
          } else {
            throw new SparkException(s"Unseen value: $label. To handle unseen values, " +
              s"set Param handleInvalid to ${CusOneHotEncoderEstimator.KEEP_INVALID}.")
          }
        }
      }
  ...
  @Since("2.3.0")
  override def copy(extra: ParamMap): CusOneHotEncoderModel = {
    val copied = new CusOneHotEncoderModel(uid, categorySizes)
    copyValues(copied, extra).setParent(parent)
  }

  @Since("2.3.0")
  override def write: MLWriter = new CusOneHotEncoderModelWriter(this)
}

@Since("2.3.0")
object CusOneHotEncoderModel extends MLReadable[CusOneHotEncoderModel] {

  private[CusOneHotEncoderModel]
  class CusOneHotEncoderModelWriter(instance: CusOneHotEncoderModel) extends MLWriter {
  ...
  private class CusOneHotEncoderModelReader extends MLReader[CusOneHotEncoderModel] {

    private val className = classOf[CusOneHotEncoderModel].getName

    override def load(path: String): CusOneHotEncoderModel = {
      ...
      val model = new CusOneHotEncoderModel(metadata.uid, categorySizes)
  ...
  @Since("2.3.0")
  override def read: MLReader[CusOneHotEncoderModel] = new CusOneHotEncoderModelReader

  @Since("2.3.0")
  override def load(path: String): CusOneHotEncoderModel = super.load(path)
}
